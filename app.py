...
""", session=session, binaural=BINAURAL, last_score=last_score, last_status=last_status,
     mood_display=mood_display, has_api=has_api)
...
def history():
    ...
    conn = sqlite3.connect("app.db")
    cur  = conn.cursor()
    cur.execute("""SELECT score, status, created_at FROM results
                   WHERE username=? ORDER BY id DESC LIMIT 20""", (session["user"],))
    rows = cur.fetchall()
    conn.close()
    return render_template_string("""
<!DOCTYPE html>
...
<style>
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Nunito',sans-serif;min-height:100vh;background:#0d0d1a;color:#fff;padding:90px 16px 60px;}
body::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse 80% 60% at 20% 40%,rgba(167,139,250,0.07) 0%,transparent 60%);pointer-events:none;}
...
.panel{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.09);border-radius:24px;padding:24px;margin-bottom:20px;}
.panel h3{font-family:'Playfair Display',serif;font-size:1.1rem;margin-bottom:16px;}
.history-list{display:flex;flex-direction:column;gap:10px;}
.history-item{display:flex;align-items:center;gap:14px;padding:14px 16px;background:rgba(255,255,255,0.04);border-radius:14px;border:1px solid rgba(255,255,255,0.07);}
.h-score{font-size:1.4rem;font-weight:900;width:52px;text-align:center;}
.h-info{flex:1;}
.h-status{font-weight:800;font-size:0.9rem;}
.h-date{color:rgba(255,255,255,0.35);font-size:0.75rem;margin-top:2px;}
.h-bar{flex:1;height:6px;background:rgba(255,255,255,0.06);border-radius:99px;overflow:hidden;max-width:120px;}
.h-bar-fill{height:100%;border-radius:99px;}
...
<div class="page-wrap">
    ...
    <p class="page-sub">Track your mental wellness journey over time</p>
    {% if rows %}
    <div class="panel">
        <h3>📊 Score Trend</h3>
    ...
    <div class="panel">
        ...
        {% for row in rows %}
        {% set pct = (row[0] / 45 * 100)|int %}
        {% set col = '#0a0' if row[0] <= 11 else ('#e6b800' if row[0] <= 22 else ('#f80' if row[0] <= 33 else '#f00')) %}
        <div class="history-item">
            <div class="h-score" style="color:{{ col }};">{{ row[0] }}</div>
            <div class="h-info">
                <div class="h-status" style="color:{{ col }};">{{ row[1] }}</div>
                <div class="h-date">{{ row[2][:16] }}</div>
            </div>
            <div class="h-bar"><div class="h-bar-fill" style="width:{{ pct }}%;background:{{ col }};"></div></div>
        </div>
        {% endfor %}
        </div>
    </div>
    <script>
    const scores={{ rows|map(attribute=0)|list|tojson }}.reverse();
    const labels={{ rows|map(attribute=2)|list|tojson }}.reverse().map(d=>d.slice(5,10));
    new Chart(document.getElementById('trendChart').getContext('2d'),{
        type:'line',
        data:{labels,datasets:[{label:'Wellness Score',data:scores,borderColor:'#a78bfa',backgroundColor:'rgba(167,139,250,0.1)',borderWidth:2,pointBackgroundColor:'#a78bfa',pointRadius:4,tension:0.4,fill:true}]},
        options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{grid:{color:'rgba(255,255,255,0.05)'},ticks:{color:'rgba(255,255,255,0.4)',font:{family:'Nunito',size:11}}},y:{grid:{color:'rgba(255,255,255,0.05)'},ticks:{color:'rgba(255,255,255,0.4)',font:{family:'Nunito',size:11}}}}}
    });
    </script>
    {% else %}
    <div class="panel"><div class="no-data">🌱 No test results yet.<br><br><a href="/test" style="color:#a78bfa;font-weight:800;">Take your first wellness check-in →</a></div></div>
    {% endif %}
</div>
</body></html>
""", rows=rows)
...
def profile():
    ...
    cur.execute("SELECT score FROM results WHERE username=? ORDER BY id DESC LIMIT 1", (session["user"],))
...
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
body{font-family:'Nunito',sans-serif;min-height:100vh;background:#0d0d1a;color:#fff;padding:90px 16px 60px;}
body::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse 80% 60% at 30% 40%,rgba(92,200,245,0.07) 0%,transparent 60%),radial-gradient(ellipse 60% 80% at 70% 20%,rgba(167,139,250,0.07) 0%,transparent 60%);pointer-events:none;}
.topnav{position:fixed;top:0;left:0;right:0;display:flex;justify-content:space-between;align-items:center;padding:14px 24px;background:rgba(13,13,26,0.85);backdrop-filter:blur(16px);border-bottom:1px solid rgba(255,255,255,0.07);z-index:100;}
.brand{font-family:'Playfair Display',serif;font-size:1.2rem;color:#fff;}
.nav-links a{color:rgba(255,255,255,0.5);text-decoration:none;font-size:0.82rem;font-weight:700;margin-left:12px;padding:6px 14px;border-radius:20px;border:1px solid rgba(255,255,255,0.1);transition:all 0.2s;}
.nav-links a:hover{color:#fff;background:rgba(255,255,255,0.08);}
.page-wrap{position:relative;z-index:1;max-width:560px;margin:0 auto;}
.avatar-wrap{text-align:center;margin-bottom:28px;}
.avatar{width:90px;height:90px;border-radius:50%;background:linear-gradient(135deg,#5bc8f5,#a78bfa);display:inline-flex;align-items:center;justify-content:center;font-size:2.5rem;font-weight:900;color:#fff;box-shadow:0 8px 32px rgba(92,200,245,0.3);margin-bottom:12px;}
.username{font-family:'Playfair Display',serif;font-size:1.6rem;}
.member-tag{color:rgba(255,255,255,0.4);font-size:0.82rem;margin-top:4px;}
.stats-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:20px;}
.stat-card{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:18px;padding:18px;text-align:center;}
.stat-val{font-size:1.8rem;font-weight:900;background:linear-gradient(135deg,#5bc8f5,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
.stat-lbl{font-size:0.72rem;color:rgba(255,255,255,0.4);margin-top:4px;font-weight:700;text-transform:uppercase;}
.info-panel{background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:20px;padding:22px;margin-bottom:16px;}
.info-row{display:flex;align-items:center;gap:12px;padding:12px 0;border-bottom:1px solid rgba(255,255,255,0.06);}
.info-row:last-child{border-bottom:none;}
.info-icon{width:36px;height:36px;border-radius:10px;background:rgba(255,255,255,0.06);display:flex;align-items:center;justify-content:center;font-size:1rem;flex-shrink:0;}
.info-label{font-size:0.75rem;color:rgba(255,255,255,0.4);font-weight:700;}
.info-value{font-size:0.92rem;font-weight:700;color:#fff;}
.action-row{display:flex;gap:10px;}
.action-btn{flex:1;padding:12px;border:none;border-radius:14px;font-family:'Nunito',sans-serif;font-weight:800;font-size:0.88rem;cursor:pointer;transition:all 0.2s;text-decoration:none;text-align:center;display:block;}
.action-btn.primary{background:linear-gradient(135deg,#5bc8f5,#a78bfa);color:#fff;}
.action-btn.danger{background:rgba(255,80,80,0.1);border:1.5px solid rgba(255,80,80,0.2);color:#ff9a9a;}
...
<div class="page-wrap">
    <div class="avatar-wrap">
        ...
        <div class="member-tag">🌿 Wellness Member</div>
    </div>
    <div class="stats-grid">
        <div class="stat-card"><div class="stat-val">{{ test_count }}</div><div class="stat-lbl">🧠 Tests Taken</div></div>
        <div class="stat-card"><div class="stat-val">{{ mood_count }}</div><div class="stat-lbl">📅 Mood Logs</div></div>
        <div class="stat-card"><div class="stat-val">{{ last_score if last_score is not none else '—' }}</div><div class="stat-lbl">📊 Last Score</div></div>
    </div>
    <div class="info-panel">
        <div class="info-row">
            <div class="info-icon">👤</div>
            <div><div class="info-label">Username</div><div class="info-value">{{ user_data[0] }}</div></div>
        </div>
        <div class="info-row">
            <div class="info-icon">📱</div>
            <div><div class="info-label">Mobile</div><div class="info-value">{{ user_data[1][:4] }}••••{{ user_data[1][-3:] }}</div></div>
        </div>
        <div class="info-row">
            <div class="info-icon">🔒</div>
    ...
    <div class="action-row">
        <a href="/history" class="action-btn primary">📈 View Full History</a>
        <a href="/logout" class="action-btn danger">👋 Log Out</a>
    </div>
</div>
</body></html>
""", session=session, user_data=user_data, test_count=test_count,
     mood_count=mood_count, last_score=last_score)
...
def detect_emotion_from_text(text: str) -> dict:
    ...
    return {"dominant": dominant, "emoji": EMOJI.get(dominant,"😐"), "color": COLOR.get(dominant,"#94a3b8"),
            "confidence": confidence, "scores": percentages, "tips": TIPS.get(dominant, TIPS["neutral"])}
...
.face-questionnaire{margin-top:20px;text-align:left;display:none;}
.face-questionnaire h4{font-size:0.9rem;font-weight:800;color:rgba(255,255,255,0.7);margin-bottom:14px;}
.fq-item{margin-bottom:14px;}
.fq-label{font-size:0.82rem;color:rgba(255,255,255,0.5);margin-bottom:6px;font-weight:700;}
.fq-options{display:flex;gap:8px;flex-wrap:wrap;}
.fq-opt{padding:6px 14px;border:1.5px solid rgba(255,255,255,0.1);border-radius:20px;background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.5);font-family:'Nunito',sans-serif;font-weight:700;font-size:0.8rem;cursor:pointer;transition:all 0.2s;}
.fq-opt.selected{background:rgba(251,191,36,0.15);border-color:#fbbf24;color:#fbbf24;}
...
<div class="page-wrap">
    ...
    <p class="page-sub">Understand what you're feeling — through text, voice, or guided self-check</p>
    <div class="panel">
        <div class="tabs">
            ...
            <button class="tab" onclick="switchTab('face',this)">📷 Self-Check</button>
        </div>
        ...
        <!-- FACE / SELF-CHECK — Fixed: uses multi-question self-report instead of brightness hack -->
        <div class="section" id="sec-face">
            <div class="face-area">
                <div class="cam-placeholder" id="camPlaceholder">
                    <div class="cam-icon">🪞</div>
                    <div style="font-weight:700;">Guided Emotion Self-Check</div>
                    <div style="font-size:0.78rem;color:rgba(255,255,255,0.3);max-width:260px;line-height:1.5;">Answer a few quick questions about your current state for accurate emotion detection</div>
                </div>
                <div class="face-questionnaire" id="faceQuestionnaire">
                    <h4>🔍 How are you right now? (select one per row)</h4>
                    <div class="fq-item">
                        <div class="fq-label">😊 Facial expression</div>
                        <div class="fq-options">
                            <button class="fq-opt" data-group="face" data-val="smiling big">😁 Big smile</button>
                            <button class="fq-opt" data-group="face" data-val="slightly smiling">🙂 Slight smile</button>
                            <button class="fq-opt" data-group="face" data-val="neutral expression">😐 Neutral</button>
                            <button class="fq-opt" data-group="face" data-val="frowning or tense">😟 Tense/frown</button>
                            <button class="fq-opt" data-group="face" data-val="tearful or crying">😢 Tearful</button>
                        </div>
                    </div>
                    <div class="fq-item">
                        <div class="fq-label">💪 Body posture</div>
                        <div class="fq-options">
                            <button class="fq-opt" data-group="body" data-val="sitting upright energetically">⚡ Upright, energetic</button>
                            <button class="fq-opt" data-group="body" data-val="relaxed and calm">😌 Relaxed</button>
                            <button class="fq-opt" data-group="body" data-val="slouched or tired">😴 Slouched/tired</button>
                            <button class="fq-opt" data-group="body" data-val="tense or fidgety">😬 Tense/fidgety</button>
                        </div>
                    </div>
                    <div class="fq-item">
                        <div class="fq-label">❤️ Chest / breathing</div>
                        <div class="fq-options">
                            <button class="fq-opt" data-group="breath" data-val="breathing deeply and steadily">🌬️ Deep & steady</button>
                            <button class="fq-opt" data-group="breath" data-val="breathing normally">😐 Normal</button>
                            <button class="fq-opt" data-group="breath" data-val="breathing fast or shallow">😰 Fast/shallow</button>
                            <button class="fq-opt" data-group="breath" data-val="chest feels tight">🫀 Chest tight</button>
                        </div>
                    </div>
                    <div class="fq-item">
                        <div class="fq-label">🧠 Mental state</div>
                        <div class="fq-options">
                            <button class="fq-opt" data-group="mind" data-val="feeling clear and positive">✨ Clear & positive</button>
                            <button class="fq-opt" data-group="mind" data-val="mind wandering or unfocused">💭 Wandering</button>
                            <button class="fq-opt" data-group="mind" data-val="feeling sad or empty">💙 Sad/empty</button>
                            <button class="fq-opt" data-group="mind" data-val="racing thoughts or worried">⚡ Racing/worried</button>
                            <button class="fq-opt" data-group="mind" data-val="frustrated or irritated">😤 Frustrated</button>
                        </div>
                    </div>
                    <button class="analyse-btn" id="faceAnalyseBtn" onclick="analyseFace()" style="margin-top:4px;" disabled>🔍 Detect My Emotion</button>
                </div>
                <button class="cam-btn" id="startCheckBtn" onclick="startSelfCheck()">🪞 Start Self-Check</button>
                <p class="face-note">No camera needed — this uses guided self-reflection for accurate results.<br>Your responses stay private and are never stored.</p>
            </div>
        </div>
    </div>
...
// ── FIXED FACE / SELF-CHECK ──────────────────────────────────
// The old implementation used image brightness → always mapped to "happy".
// New approach: guided multi-dimension self-report questionnaire.
// Each answer contributes semantic keywords → run through same emotion lexicon.
...
function startSelfCheck(){
    ...
    document.getElementById('faceQuestionnaire').style.display='block';
    // Wire up option buttons
    document.querySelectorAll('.fq-opt').forEach(btn=>{
        btn.addEventListener('click',()=>{
            const group=btn.dataset.group;
            document.querySelectorAll(`.fq-opt[data-group="${group}"]`).forEach(b=>b.classList.remove('selected'));
            btn.classList.add('selected');
            fqSelections[group]=btn.dataset.val;
            // Enable analyse btn if all 4 groups selected
            const allSelected=Object.keys(fqSelections).length>=4;
            document.getElementById('faceAnalyseBtn').disabled=!allSelected;
        });
    });
}

async function analyseFace(){
    // Build a rich descriptive sentence from the 4 self-report dimensions
    const parts=Object.values(fqSelections);
    const compositeText=`I am ${parts.join(', ')}. ${buildFaceNarrative(fqSelections)}`;
    const btn=document.getElementById('faceAnalyseBtn');
    btn.disabled=true;btn.textContent='⏳ Analysing...';
    const res=await fetch('/api/emotion',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text:compositeText})});
    const d=await res.json();
    btn.disabled=false;btn.textContent='🔍 Detect My Emotion';
    showResult(d);
}

function buildFaceNarrative(sel){
    // Map combinations to richer emotional language for better lexicon matching
    const narratives=[];
    const face=sel.face||'';const body=sel.body||'';const breath=sel.breath||'';const mind=sel.mind||'';
    if(face.includes('smiling big')||face.includes('slightly smiling'))narratives.push('I feel happy and joyful');
    if(face.includes('frowning')||face.includes('tense'))narratives.push('I feel stressed and tense');
    if(face.includes('tearful'))narratives.push('I feel sad and crying');
    if(body.includes('energetically'))narratives.push('I have lots of energy and feel motivated');
    if(body.includes('relaxed'))narratives.push('I feel calm and peaceful');
    if(body.includes('slouched')||body.includes('tired'))narratives.push('I feel tired and exhausted');
    if(body.includes('fidgety'))narratives.push('I feel anxious and restless');
    if(breath.includes('deep'))narratives.push('I am breathing calmly and feeling relaxed');
    if(breath.includes('fast')||breath.includes('shallow'))narratives.push('I feel anxious and worried my heart is racing');
    if(breath.includes('tight'))narratives.push('I feel panic and my chest is tight');
    if(mind.includes('clear'))narratives.push('I feel wonderful and amazing, content');
    if(mind.includes('wandering'))narratives.push('I have trouble concentrating and feel unfocused');
    if(mind.includes('sad'))narratives.push('I feel sad and empty and hopeless');
    if(mind.includes('racing'))narratives.push('I feel anxious and worried with racing thoughts');
    if(mind.includes('frustrated'))narratives.push('I feel angry and frustrated and irritated');
...
def openai_chat_reply(messages_history: list, user_msg: str) -> str:
    ...
    system_prompt = """You are Sage, a compassionate AI mental wellness companion built into MindSpace. Your role is to listen with empathy, provide evidence-based coping strategies (CBT, mindfulness, breathing), give emotional support, and gently encourage professional help when needed. Keep responses warm, concise (2-4 sentences), and conversational. NEVER diagnose or prescribe. If someone expresses suicidal ideation, provide Indian crisis helplines: iCall 9152987821, Vandrevala Foundation 1860-2662-345."""
    ...
    payload = json.dumps({"model": "gpt-4o-mini", "max_tokens": 300, "temperature": 0.8, "messages": api_messages}).encode("utf-8")
    try:
...
body::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse 80% 60% at 20% 40%,rgba(99,102,241,0.08) 0%,transparent 60%);pointer-events:none;z-index:0;}
...
function addMsg(text,role){const div=document.createElement('div');div.className='msg '+role;div.innerHTML=`<div class="bubble">${text.replace(/\\n/g,'<br>')}</div><div class="msg-time">${nowTime()}</div>`;messagesEl.appendChild(div);messagesEl.scrollTop=messagesEl.scrollHeight;chatHistory.push({role:role==='bot'?'assistant':'user',content:text});}
...
async function sendMsg(){
    ...
    try{const res=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text,history:chatHistory.slice(-12)})});const d=await res.json();removeTyping();addMsg(d.reply,'bot');}
...
<div class="page-wrap">
    ...
    <div class="chart-card">
        ...
        <div id="chartWeekly" style="position:relative;height:260px;"><canvas id="weekChart"></canvas></div>
        <div id="chartMonthly" style="position:relative;height:260px;display:none;"><canvas id="monthChart"></canvas></div>
        {% if total_logs > 0 %}
...
# ── GAMES (Chess + Sudoku) ────────────────────────────────────
# [Kept identical to original — omitted here for brevity, paste from original]

@app.route("/games")
def games():
    if "user" not in session:
        return redirect("/login")
    # Full games page — copy the complete /games route from the original code
    # (it is complete and correct in your document)
    return redirect("/")  # placeholder — paste original games() body here
...
def puzzle():
    if "user" not in session:
        return redirect("/login")
    # Full puzzle page — copy from original code (complete and correct)
    return redirect("/")  # placeholder — paste original puzzle() body here
...
def activity():
    if "user" not in session:
        return redirect("/login")
    return render_template_string(ACTIVITY_PAGE_HTML)

ACTIVITY_PAGE_HTML = """<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
    ...
    <div class="section-panel" id="panel-workout">
        <div class="workout-wrap">
            <div class="builder-row">
                ...
                <select class="builder-select" id="wGoal">
                    <option value="calm">Stress Relief</option>
                ...
                <select class="builder-select" id="wDur">
...
<div class="pose-modal" id="poseModal">
    <div class="pose-modal-box">
        ...
        <div class="modal-timer">
            <div class="timer-count" id="mTimerCount">--</div>
            <div class="timer-label">seconds remaining</div>
        </div>
        <div class="modal-steps" id="mSteps"></div>
        <div class="modal-actions">
            <button class="modal-btn primary" id="mTimerBtn" onclick="startPoseTimer()">▶ Start Timer</button>
            <button class="modal-btn secondary" onclick="closePoseModal()">✕ Close</button>
        </div>
    </div>
</div>
...
const YOGA_POSES=[
  {icon:"🧘",name:"Mountain Pose",duration:"60",benefit:"yoga",tag:"Grounding",steps:["Stand tall, feet hip-width apart","Arms relaxed at sides, palms forward","Breathe deeply, feel rooted to the ground","Hold for 60 seconds, eyes softly closed"]},
  {icon:"🌅",name:"Warrior I",duration:"45",benefit:"yoga",tag:"Strength",steps:["Step right foot forward into a lunge","Raise both arms overhead, palms facing","Back foot at 45°, hips square forward","Hold 45s, switch sides"]},
  {icon:"🌊",name:"Child's Pose",duration:"90",benefit:"yoga",tag:"Calming",steps:["Kneel and sit back on heels","Fold forward, arms stretched ahead","Forehead rests on mat","Breathe deeply into the back body"]},
  {icon:"🌿",name:"Downward Dog",duration:"60",benefit:"yoga",tag:"Full Body",steps:["Start on hands and knees","Push hips up and back into inverted V","Heels press toward floor","Pedal feet gently to release hamstrings"]},
  {icon:"⚖️",name:"Tree Pose",duration:"30",benefit:"yoga",tag:"Balance",steps:["Stand on one foot firmly","Place other foot on inner thigh","Hands in prayer at chest","Focus on a fixed point ahead"]},
  {icon:"🦋",name:"Butterfly Pose",duration:"90",benefit:"yoga",tag:"Hip Opener",steps:["Sit with soles of feet together","Hold feet with both hands","Gently flap knees up and down","Fold forward for a deeper stretch"]},
  {icon:"🐍",name:"Cobra Pose",duration:"30",benefit:"yoga",tag:"Backbend",steps:["Lie face down, palms under shoulders","Press up, lifting chest off mat","Elbows slightly bent, shoulders down","Hold and breathe into the chest"]},
  {icon:"🌙",name:"Legs Up Wall",duration:"300",benefit:"yoga",tag:"Restorative",steps:["Sit sideways against a wall","Swing legs up as you lie back","Arms relaxed by your sides","Close eyes, breathe slowly for 5 min"]}
];
const STRETCH_POSES=[
  {icon:"🤸",name:"Neck Rolls",duration:"60",benefit:"stretch",tag:"Neck Relief",steps:["Sit comfortably, spine tall","Drop right ear to right shoulder","Slowly roll chin to chest","Continue to left side, repeat 3×"]},
  {icon:"🙆",name:"Chest Opener",duration:"45",benefit:"stretch",tag:"Posture",steps:["Clasp hands behind your back","Squeeze shoulder blades together","Lift chest and look slightly up","Hold and breathe deeply"]},
  {icon:"🦵",name:"Hamstring Stretch",duration:"45",benefit:"stretch",tag:"Lower Body",steps:["Sit on floor, one leg extended","Other foot against inner thigh","Reach toward extended foot","Keep back straight, not rounded"]},
  {icon:"🔄",name:"Seated Spinal Twist",duration:"45",benefit:"stretch",tag:"Spine",steps:["Sit cross-legged or with legs extended","Place right hand behind you","Left hand on right knee","Twist right on inhale, deepen on exhale"]},
  {icon:"🤾",name:"Hip Flexor Stretch",duration:"45",benefit:"stretch",tag:"Hips",steps:["Kneel on right knee, left foot forward","Sink hips forward and down","Keep torso upright, hands on knee","Feel the deep stretch in right hip"]},
  {icon:"💫",name:"Figure-4 Stretch",duration:"60",benefit:"stretch",tag:"Glutes",steps:["Lie on back, knees bent","Cross right ankle over left knee","Thread hands behind left thigh","Pull toward chest gently"]}
];
const STRENGTH_POSES=[
  {icon:"💪",name:"Push-Ups",duration:"45",benefit:"strength",tag:"Upper Body",steps:["Hands slightly wider than shoulders","Lower chest to near floor","Keep core engaged throughout","Push back up with full control"]},
  {icon:"🏋️",name:"Bodyweight Squats",duration:"45",benefit:"strength",tag:"Legs",steps:["Feet shoulder-width apart, toes out slightly","Push hips back as you bend knees","Thighs parallel to floor at bottom","Drive through heels to stand"]},
  {icon:"🌉",name:"Glute Bridge",duration:"45",benefit:"strength",tag:"Core & Glutes",steps:["Lie on back, knees bent, feet flat","Press through heels to lift hips","Squeeze glutes firmly at the top","Lower slowly with control"]},
  {icon:"🦅",name:"Plank",duration:"30",benefit:"strength",tag:"Core",steps:["Forearms on floor, elbows under shoulders","Body in a straight line head to heels","Engage core, don't let hips sag","Breathe steadily throughout"]},
  {icon:"🚶",name:"Reverse Lunges",duration:"45",benefit:"strength",tag:"Legs",steps:["Stand with feet together","Step one foot back into a lunge","Front knee over ankle, not toe","Push off back foot to return"]},
  {icon:"🦸",name:"Superman Hold",duration:"30",benefit:"strength",tag:"Back",steps:["Lie face down, arms extended overhead","Simultaneously lift arms and legs","Hold for 2 seconds at the top","Lower slowly and repeat 10×"]}
];

const allPoses={yoga:YOGA_POSES,stretch:STRETCH_POSES,strength:STRENGTH_POSES};

function renderPoses(poses,gridId,type){
  const grid=document.getElementById(gridId);
  grid.innerHTML=poses.map((p,i)=>`
    <div class="pose-card ${type}" onclick="openPoseModal(${i},'${type}')">
      <div class="pose-icon">${p.icon}</div>
      <div class="pose-name">${p.name}</div>
      <div class="pose-duration">⏱ ${p.duration}s</div>
      <span class="pose-benefit tag-${type}">${p.tag}</span>
    </div>`).join("");
}
renderPoses(YOGA_POSES,"yogaGrid","yoga");
renderPoses(STRETCH_POSES,"stretchGrid","stretch");
renderPoses(STRENGTH_POSES,"strengthGrid","strength");

let poseTimerInterval=null,poseTimeLeft=0;
function openPoseModal(idx,type){
  const p=allPoses[type][idx];
  document.getElementById("mIcon").textContent=p.icon;
  document.getElementById("mTitle").textContent=p.name;
  document.getElementById("mSub").textContent=`${p.duration}s · ${p.tag}`;
  poseTimeLeft=parseInt(p.duration)||60;
  document.getElementById("mTimerCount").textContent=poseTimeLeft;
  document.getElementById("mTimerBtn").textContent="▶ Start Timer";
  if(poseTimerInterval){clearInterval(poseTimerInterval);poseTimerInterval=null;}
  document.getElementById("mSteps").innerHTML=p.steps.map((s,i)=>`<div class="modal-step"><div class="step-num">${i+1}</div><div class="step-text">${s}</div></div>`).join("");
  document.getElementById("poseModal").classList.add("show");
}
function closePoseModal(){document.getElementById("poseModal").classList.remove("show");if(poseTimerInterval){clearInterval(poseTimerInterval);poseTimerInterval=null;}}
function startPoseTimer(){
  if(poseTimerInterval){clearInterval(poseTimerInterval);poseTimerInterval=null;document.getElementById("mTimerBtn").textContent="▶ Start Timer";return;}
  document.getElementById("mTimerBtn").textContent="⏸ Pause";
  poseTimerInterval=setInterval(()=>{poseTimeLeft--;document.getElementById("mTimerCount").textContent=poseTimeLeft;if(poseTimeLeft<=0){clearInterval(poseTimerInterval);poseTimerInterval=null;document.getElementById("mTimerCount").textContent="✅";document.getElementById("mTimerBtn").textContent="✅ Done!";}},1000);
}

// Breathing engine
const BREATH_TYPES={
  "478":{phases:[{n:"Inhale",t:4,cls:"inhale"},{n:"Hold",t:7,cls:"hold"},{n:"Exhale",t:8,cls:"exhale"}],desc:"4-7-8: Inhale 4s, Hold 7s, Exhale 8s — reduces anxiety fast"},
  "box":{phases:[{n:"Inhale",t:4,cls:"inhale"},{n:"Hold",t:4,cls:"hold"},{n:"Exhale",t:4,cls:"exhale"},{n:"Hold",t:4,cls:"hold"}],desc:"Box Breathing: 4s each phase — used by Navy SEALs for focus"},
  "belly":{phases:[{n:"Inhale",t:5,cls:"inhale"},{n:"Exhale",t:7,cls:"exhale"}],desc:"Belly Breathing: 5s in, 7s out — activates the parasympathetic system"},
  "coherent":{phases:[{n:"Inhale",t:5,cls:"inhale"},{n:"Exhale",t:5,cls:"exhale"}],desc:"Coherent Breathing: 5s in, 5s out — balances heart rate variability"}
};
let breathType="478",breathRunning=false,breathInterval=null,breathPhaseIdx=0,breathSecondsLeft=0,breathCycles=0;
...
function resetBreath(){stopBreath();breathPhaseIdx=0;breathCycles=0;breathSecondsLeft=0;document.getElementById("breathPhase").textContent="Press Start";document.getElementById("breathCountNum").textContent="";document.getElementById("breathCircle").className="breath-circle";document.getElementById("cyclesCount").textContent="Cycles completed: 0";document.getElementById("breathStart").textContent="▶ Start";}
function stopBreath(){if(breathInterval){clearInterval(breathInterval);breathInterval=null;}breathRunning=false;}
function toggleBreath(){if(breathRunning){stopBreath();document.getElementById("breathStart").textContent="▶ Resume";}else{breathRunning=true;document.getElementById("breathStart").textContent="⏸ Pause";runBreathPhase();}}
function runBreathPhase(){
  const phases=BREATH_TYPES[breathType].phases;const phase=phases[breathPhaseIdx];
  breathSecondsLeft=phase.t;document.getElementById("breathPhase").textContent=phase.n;document.getElementById("breathCountNum").textContent=breathSecondsLeft;document.getElementById("breathCircle").className="breath-circle "+phase.cls;
  breathInterval=setInterval(()=>{breathSecondsLeft--;document.getElementById("breathCountNum").textContent=breathSecondsLeft;if(breathSecondsLeft<=0){clearInterval(breathInterval);breathPhaseIdx++;if(breathPhaseIdx>=phases.length){breathPhaseIdx=0;breathCycles++;document.getElementById("cyclesCount").textContent=`Cycles completed: ${breathCycles}`;}if(breathRunning)runBreathPhase();}},1000);
}

// Workout plan builder
const EXERCISE_POOL={
  calm:[{icon:"🧘",name:"Child's Pose",detail:"60 seconds"},{icon:"🌊",name:"Legs Up Wall",detail:"3 minutes"},{icon:"🌬️",name:"4-7-8 Breathing",detail:"5 cycles"},{icon:"🦋",name:"Butterfly Pose",detail:"90 seconds"},{icon:"🔄",name:"Seated Spinal Twist",detail:"45s each side"},{icon:"🌙",name:"Savasana",detail:"3 minutes"}],
  energy:[{icon:"⭐",name:"Jumping Jacks",detail:"3×30 reps"},{icon:"🏃",name:"High Knees",detail:"3×30 seconds"},{icon:"💪",name:"Push-Ups",detail:"3×12 reps"},{icon:"🏋️",name:"Squats",detail:"3×15 reps"},{icon:"🔥",name:"Burpees",detail:"3×8 reps"},{icon:"🦅",name:"Plank",detail:"3×30 seconds"}],
  strength:[{icon:"💪",name:"Push-Ups",detail:"4×15 reps"},{icon:"🏋️",name:"Squats",detail:"4×20 reps"},{icon:"🌉",name:"Glute Bridge",detail:"3×15 reps"},{icon:"🚶",name:"Reverse Lunges",detail:"3×10 each"},{icon:"🦸",name:"Superman Hold",detail:"3×10 reps"},{icon:"🦅",name:"Plank",detail:"3×45 seconds"}],
  flex:[{icon:"🤸",name:"Neck Rolls",detail:"60 seconds"},{icon:"🦵",name:"Hamstring Stretch",detail:"45s each"},{icon:"🤾",name:"Hip Flexor Stretch",detail:"45s each"},{icon:"💫",name:"Figure-4 Stretch",detail:"60s each"},{icon:"🙆",name:"Chest Opener",detail:"45 seconds"},{icon:"🦋",name:"Butterfly Pose",detail:"90 seconds"}]
};
function buildWorkout(){
  const goal=document.getElementById("wGoal").value;const dur=parseInt(document.getElementById("wDur").value);
  const count=dur<=10?3:dur<=20?5:7;
  const pool=[...EXERCISE_POOL[goal]].sort(()=>Math.random()-0.5).slice(0,count);
  document.getElementById("workoutPlan").innerHTML=pool.map((e,i)=>`
    <div class="workout-exercise">
      <div class="ex-num">${i+1}</div>
      <div class="ex-info"><div class="ex-name">${e.name}</div><div class="ex-detail">${e.detail}</div></div>
